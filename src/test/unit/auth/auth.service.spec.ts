...
describe('AuthService', () => {
    ...
    describe('authenticateWithUserCredentials', () => {
        it('deve retornar tokens quando credenciais validas e usuario existe no banco', async () => {
            ...
            expect(authProvider.authenticateWithCredentials).toHaveBeenCalledWith(dto);
            expect(tokenVerifier.verifyIdToken).toHaveBeenCalledWith(mockTokens.idToken);
            expect(userService.validateUserExists).toHaveBeenCalledWith(mockDecoded.uid);
            expect(userService.syncUserWithFirebase).toHaveBeenCalledWith(mockDecoded.uid);
            expect(result).toEqual(mockTokens);
        });
        it('deve propagar ForbiddenException quando usuario nao existe no banco', async () => {
            userService.validateUserExists.mockRejectedValue(new ForbiddenException('User not found in database'));
            await expect(service.authenticateWithUserCredentials({
    ...
    describe('refreshToken', () => {
        ...
        it('deve lancar BadRequestException quando refresh token ausente', async () => {
            await expect(service.refreshToken({} as any)).rejects.toThrow(BadRequestException);
        });
    });
    describe('logout', () => {
        it('deve chamar revokeRefreshTokens com o uid', async () => {
            await service.logout('firebase-uid-1');
            expect(authProvider.revokeRefreshTokens).toHaveBeenCalledWith('firebase-uid-1');
        });
    });
});
