...
describe('AuthController (HTTP)', () => {
    ...
    describe('POST /auth/login', () => {
        it('deve retornar 201 e tokens quando credenciais validas', () => {
            authService.authenticateWithUserCredentials.mockResolvedValue(mockLoginResponse);
            ...
            return request(app.getHttpServer())
                ...
                expect(res.body).toHaveProperty('refreshToken', mockLoginResponse.refreshToken);
                expect(authService.authenticateWithUserCredentials).toHaveBeenCalledWith(dto);
            });
        });
        it('deve retornar 403 quando usuario nao existe no banco ou inativo', () => {
            authService.authenticateWithUserCredentials.mockRejectedValue(new ForbiddenException('User not found in database'));
            return request(app.getHttpServer())
                .post('/auth/login')
