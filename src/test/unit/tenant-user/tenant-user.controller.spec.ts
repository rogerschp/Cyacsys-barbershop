...
describe('TenantUserController (HTTP)', () => {
  ...
  beforeAll(async () => {
    ...
    tenantUserService = moduleFixture.get(
      TenantUserService,
    ) as jest.Mocked<TenantUserService>;
  });
  ...
  describe('POST /tenants/:tenantId/members', () => {
    it('deve retornar 201 e o vinculo criado', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(tenantUserService.addUserToTenant).toHaveBeenCalledWith(
            'user-uuid',
            'tenant-uuid',
    ...
    it('deve retornar 409 quando usuario ja vinculado', () => {
      tenantUserService.addUserToTenant.mockRejectedValue(
  ...
  describe('GET /tenants/:tenantId/members/:userId', () => {
    it('deve retornar 200 e o vinculo', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body).toHaveProperty('id', 'link-uuid');
          expect(res.body).toHaveProperty('role', TenantUserRole.OWNER);
          expect(tenantUserService.getMembership).toHaveBeenCalledWith(
            'tenant-uuid',
            'user-uuid',
          );
        });
    ...
    it('deve retornar 404 quando vinculo nao existe', () => {
      tenantUserService.getMembership.mockRejectedValue(
  ...
  describe('DELETE /tenants/:tenantId/members/:userId', () => {
    it('deve retornar 200 ao remover o vinculo', async () => {
      ...
      expect(tenantUserService.removeUserFromTenant).toHaveBeenCalledWith(
        'user-uuid',
        'tenant-uuid',
      );
    });
    ...
    it('deve retornar 404 quando vinculo nao existe', () => {
      tenantUserService.removeUserFromTenant.mockRejectedValue(
        new NotFoundException('No membership found'),
      );
