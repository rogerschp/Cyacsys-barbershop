...
describe('CreateServiceUseCase', () => {
    ...
    describe('run', () => {
        ...
        it('deve lançar BusinessRuleException quando preço é negativo', async () => {
            await expect(useCase.run(tenantId, { ...validDto, price: -1 }, createdBy)).rejects.toThrow(BusinessRuleException);
            expect(serviceRepository.create).not.toHaveBeenCalled();
        });
        it('deve lançar BusinessRuleException quando duração é menor que 5 min', async () => {
            await expect(useCase.run(tenantId, { ...validDto, durationInMinutes: 3 }, createdBy)).rejects.toThrow(BusinessRuleException);
            expect(serviceRepository.create).not.toHaveBeenCalled();
        });
        it('deve normalizar nome com trim ao criar', async () => {
            await useCase.run(tenantId, { ...validDto, name: '  Corte masculino  ' }, createdBy);
            expect(serviceRepository.findNonDeletedByName).toHaveBeenCalledWith(tenantId, 'Corte masculino');
            expect(serviceRepository.create).toHaveBeenCalledWith(expect.objectContaining({ name: 'Corte masculino' }));
        });
        it('deve passar description null quando não informada', async () => {
            ...
            expect(serviceRepository.create).toHaveBeenCalledWith(expect.objectContaining({ description: null }));
        });
    });
});
