...
describe('UpdateServiceUseCase', () => {
    ...
    describe('run', () => {
        it('deve atualizar nome quando único no tenant', async () => {
            ...
            expect(serviceRepository.findNonDeletedByName).toHaveBeenCalledWith(tenantId, 'Corte novo', serviceId);
            expect(serviceRepository.update).toHaveBeenCalledWith(serviceId, tenantId, { name: 'Corte novo' });
            expect(result).toEqual(updated);
        });
        it('deve lançar NotFoundException quando serviço não existe', async () => {
            serviceRepository.findById.mockResolvedValue(null);
            await expect(useCase.run(tenantId, serviceId, { name: 'Corte' })).rejects.toThrow(NotFoundException);
        ...
        it('deve lançar BusinessRuleException quando novo nome já existe', async () => {
            ...
            await expect(useCase.run(tenantId, serviceId, { name: 'Corte duplicado' })).rejects.toThrow(BusinessRuleException);
            expect(serviceRepository.update).not.toHaveBeenCalled();
        });
        it('deve lançar BusinessRuleException quando preço é negativo', async () => {
            await expect(useCase.run(tenantId, serviceId, { price: -1 })).rejects.toThrow(BusinessRuleException);
            expect(serviceRepository.update).not.toHaveBeenCalled();
        });
        it('deve lançar BusinessRuleException quando duração menor que 5 min', async () => {
            await expect(useCase.run(tenantId, serviceId, { durationInMinutes: 2 })).rejects.toThrow(BusinessRuleException);
            expect(serviceRepository.update).not.toHaveBeenCalled();
        });
        ...
        it('deve atualizar vários campos de uma vez', async () => {
            ...
            expect(serviceRepository.update).toHaveBeenCalledWith(serviceId, tenantId, expect.objectContaining({
                name: 'Corte premium',
                price: '60.00',
                durationInMinutes: 45,
            }));
