    ...
    describe('POST /tenants/:tenantId/services', () => {
        it('deve retornar 201 e o serviço criado', () => {
            ...
            return request(app.getHttpServer())
                ...
                expect(createServiceUseCase.run).toHaveBeenCalledWith(tenantId, expect.objectContaining({
                    name: 'Corte masculino',
                    price: 45,
                    durationInMinutes: 30,
                }), 'user-uuid-123');
        ...
        it('deve retornar 400 quando regra de negócio violada', () => {
            createServiceUseCase.run.mockRejectedValue(new BusinessRuleException('SERVICE_NAME_ALREADY_EXISTS', 'Nome já existe'));
            return request(app.getHttpServer())
                .post(`/tenants/${tenantId}/services`)
                .send({
                name: 'Corte masculino',
        ...
        it('deve retornar 404 quando tenant não existe', () => {
            createServiceUseCase.run.mockRejectedValue(new NotFoundException('Tenant not found'));
            return request(app.getHttpServer())
                .post(`/tenants/${tenantId}/services`)
                .send({
                name: 'Corte',
    ...
    describe('GET /tenants/:tenantId/services/:id', () => {
        it('deve retornar 200 e o serviço quando existe', () => {
            ...
            return request(app.getHttpServer())
                ...
                expect(res.body).toHaveProperty('id', serviceId);
                expect(res.body).toHaveProperty('name', 'Corte masculino');
                expect(getServiceUseCase.run).toHaveBeenCalledWith(tenantId, serviceId);
            });
        });
        it('deve retornar 404 quando serviço não existe', () => {
            getServiceUseCase.run.mockRejectedValue(new NotFoundException('Service not found'));
            return request(app.getHttpServer())
                .get(`/tenants/${tenantId}/services/${serviceId}`)
                .expect(404);
        });
    ...
    describe('PATCH /tenants/:tenantId/services/:id', () => {
        it('deve retornar 200 e o serviço atualizado', () => {
            ...
            return request(app.getHttpServer())
                ...
                expect(updateServiceUseCase.run).toHaveBeenCalledWith(tenantId, serviceId, { name: 'Corte premium' }, 'user-uuid-123');
            });
        });
        it('deve retornar 404 quando serviço não existe', () => {
            updateServiceUseCase.run.mockRejectedValue(new NotFoundException('Service not found'));
            return request(app.getHttpServer())
                .patch(`/tenants/${tenantId}/services/${serviceId}`)
                .send({ name: 'Corte' })
                .expect(404);
    ...
    describe('PATCH /tenants/:tenantId/services/:id/deactivate', () => {
        it('deve retornar 200 e o serviço desativado', () => {
            ...
            return request(app.getHttpServer())
                ...
                expect(deactivateServiceUseCase.run).toHaveBeenCalledWith(tenantId, serviceId, 'user-uuid-123');
            });
        });
        it('deve retornar 404 quando serviço não existe', () => {
            deactivateServiceUseCase.run.mockRejectedValue(new NotFoundException('Service not found'));
            return request(app.getHttpServer())
                .patch(`/tenants/${tenantId}/services/${serviceId}/deactivate`)
                .expect(404);
        });
