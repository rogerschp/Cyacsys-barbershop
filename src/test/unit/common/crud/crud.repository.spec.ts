...
class TestBaseRepository extends BaseRepository<MockEntity, {
    name: string;
}, {
    name?: string;
}> {
    ...
    async findPaginated(options: PaginatedOptionsDto): Promise<PaginatedResponseDto<MockEntity>> {
        return new PaginatedResponseDto({ data: [], total: 0 }, options);
    }
    async findOne(id: string, tenantId: string): Promise<MockEntity> {
        return ((this.repo as any).findOneMock?.(id, tenantId) ??
...
describe('BaseRepository', () => {
    ...
    describe('create', () => {
        it('deve criar entidade com tenantId e salvar', async () => {
            ...
            const result = await repository.create({ name: 'Test Entity' }, 'tenant-1');
    ...
    describe('update', () => {
        it('deve chamar findOne, update e findOne novamente', async () => {
            ...
            const result = await repository.update('entity-1', { name: 'Updated' }, 'tenant-1');
            expect((typeOrmRepo as any).findOneMock).toHaveBeenCalledTimes(2);
            expect((typeOrmRepo as any).findOneMock).toHaveBeenCalledWith('entity-1', 'tenant-1');
            expect(typeOrmRepo.update).toHaveBeenCalledWith({ id: 'entity-1', tenantId: 'tenant-1' }, { name: 'Updated' });
