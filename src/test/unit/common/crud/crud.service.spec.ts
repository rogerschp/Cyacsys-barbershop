...
describe('CrudService', () => {
    let service: CrudService<MockEntity, ICrudRepository<MockEntity, {
        name: string;
    }, {
        name?: string;
    }>, {
    ...
    let repository: jest.Mocked<ICrudRepository<MockEntity, {
        name: string;
    }, {
        name?: string;
    }>>;
    ...
    describe('findPaginated', () => {
        it('deve delegar ao repository e retornar resposta paginada', async () => {
            ...
            expect(repository.findPaginated).toHaveBeenCalledWith(options, 'tenant-1');
    ...
    describe('create', () => {
        it('deve delegar ao repository e retornar a entidade criada', async () => {
            ...
            expect(repository.create).toHaveBeenCalledWith({ name: 'Test Entity' }, 'tenant-1');
            expect(result).toEqual(mockEntity);
        });
    });
    describe('update', () => {
        it('deve delegar ao repository e retornar a entidade atualizada', async () => {
            ...
            const result = await service.update('entity-1', { name: 'Updated' }, 'tenant-1');
            expect(repository.update).toHaveBeenCalledWith('entity-1', { name: 'Updated' }, 'tenant-1');
