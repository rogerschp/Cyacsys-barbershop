...
describe('UpdateBarberProfileUseCase', () => {
    ...
    describe('run (caller não BARBER)', () => {
        ...
        it('deve lançar NotFoundException quando perfil não existe', async () => {
            ...
            await expect(useCase.run(tenantId, profileId, { displayName: 'X' }, currentUserId)).rejects.toThrow(NotFoundException);
        ...
        it('deve lançar BusinessRuleException quando experienceYears negativo', async () => {
            await expect(useCase.run(tenantId, profileId, { experienceYears: -1 }, currentUserId, TenantUserRole.ADMIN)).rejects.toThrow(BusinessRuleException);
            expect(barberProfileRepository.update).not.toHaveBeenCalled();
        });
        it('deve retornar existente quando dto vazio', async () => {
            const result = await useCase.run(tenantId, profileId, {}, currentUserId, TenantUserRole.ADMIN);
            expect(barberProfileRepository.update).not.toHaveBeenCalled();
    ...
    describe('run (caller BARBER)', () => {
        ...
        it('deve lançar ForbiddenException quando BARBER tenta editar perfil de outro', async () => {
            ...
            await expect(useCase.run(tenantId, profileId, { bio: 'Nova bio' }, currentUserId, TenantUserRole.BARBER)).rejects.toThrow(ForbiddenException);
            expect(barberProfileRepository.update).not.toHaveBeenCalled();
        });
        it('deve ignorar displayName e experienceYears quando caller é BARBER', async () => {
            ...
            await useCase.run(tenantId, profileId, dto, currentUserId, TenantUserRole.BARBER);
            expect(barberProfileRepository.update).toHaveBeenCalledWith(profileId, tenantId, { bio: 'Só bio' });
        });
    });
});
