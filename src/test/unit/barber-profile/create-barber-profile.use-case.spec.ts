...
describe('CreateBarberProfileUseCase', () => {
    ...
    describe('run', () => {
        it('deve criar perfil quando tenantUser existe, é BARBER e não tem perfil', async () => {
            ...
            expect(barberProfileRepository.findByTenantUserIdNonDeleted).toHaveBeenCalledWith(tenantId, tenantUserId);
        ...
        it('deve lançar NotFoundException quando tenantUser não existe', async () => {
            tenantUserService.getByIdAndTenant.mockRejectedValue(new NotFoundException('TENANT_USER_NOT_FOUND'));
        ...
        it('deve lançar BusinessRuleException quando role do tenantUser não é BARBER', async () => {
            ...
            await expect(useCase.run(tenantId, validDto, createdBy)).rejects.toThrow(BusinessRuleException);
            expect(barberProfileRepository.create).not.toHaveBeenCalled();
        });
        it('deve lançar BusinessRuleException quando já existe perfil para o tenantUserId', async () => {
            barberProfileRepository.findByTenantUserIdNonDeleted.mockResolvedValue(mockProfile);
            await expect(useCase.run(tenantId, validDto, createdBy)).rejects.toThrow(BusinessRuleException);
            expect(barberProfileRepository.create).not.toHaveBeenCalled();
        });
        it('deve lançar BusinessRuleException quando experienceYears é negativo', async () => {
            await expect(useCase.run(tenantId, { ...validDto, experienceYears: -1 }, createdBy)).rejects.toThrow(BusinessRuleException);
            expect(barberProfileRepository.create).not.toHaveBeenCalled();
        });
        it('deve normalizar displayName com trim', async () => {
            await useCase.run(tenantId, { ...validDto, displayName: '  João Barbeiro  ' }, createdBy);
            expect(barberProfileRepository.create).toHaveBeenCalledWith(expect.objectContaining({ displayName: 'João Barbeiro' }));
        });
        it('deve passar bio null quando não informada', async () => {
            ...
            expect(barberProfileRepository.create).toHaveBeenCalledWith(expect.objectContaining({ bio: null }));
        });
    });
});
