...
import { CreateBarberProfileDto } from '../../../modules/barber-profile/dto/create-barber-profile.dto';
...
describe('CreateBarberProfileUseCase', () => {
  ...
  beforeEach(async () => {
    barberProfileRepository = {
      create: jest
        .fn<() => Promise<BarberProfileEntity>>()
        .mockResolvedValue(mockProfile),
      findByTenantUserIdNonDeleted: jest
  ...
  describe('run', () => {
    it('deve criar perfil quando tenantUser existe, é BARBER e não tem perfil', async () => {
      ...
      expect(
        barberProfileRepository.findByTenantUserIdNonDeleted,
      ).toHaveBeenCalledWith(tenantId, tenantUserId);
    ...
    it('deve criar perfil quando tenantUser é OWNER (owner também pode atuar como BARBER)', async () => {
      ...
      const result = await useCase.run(tenantId, validDto, createdBy);
      expect(barberProfileRepository.create).toHaveBeenCalled();
      expect(result).toEqual(mockProfile);
    });
