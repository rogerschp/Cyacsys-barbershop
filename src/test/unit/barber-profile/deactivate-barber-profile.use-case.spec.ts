...
describe('DeactivateBarberProfileUseCase', () => {
    ...
    beforeEach(async () => {
        ...
        useCase = module.get<DeactivateBarberProfileUseCase>(DeactivateBarberProfileUseCase);
    });
    ...
    describe('run', () => {
        it('deve desativar perfil e retornar entidade atualizada', async () => {
            ...
            expect(barberProfileRepository.update).toHaveBeenCalledWith(profileId, tenantId, { isActive: false });
        ...
        it('deve lançar NotFoundException quando perfil não existe', async () => {
            ...
            await expect(useCase.run(tenantId, profileId, performedBy)).rejects.toThrow(NotFoundException);
