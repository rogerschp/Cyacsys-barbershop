...
describe('BarberProfileController (HTTP)', () => {
  ...
  beforeAll(async () => {
    ...
    createBarberProfileUseCase = moduleFixture.get(
      CreateBarberProfileUseCase,
    ) as jest.Mocked<CreateBarberProfileUseCase>;
    updateBarberProfileUseCase = moduleFixture.get(
      UpdateBarberProfileUseCase,
    ) as jest.Mocked<UpdateBarberProfileUseCase>;
    deactivateBarberProfileUseCase = moduleFixture.get(
      DeactivateBarberProfileUseCase,
    ) as jest.Mocked<DeactivateBarberProfileUseCase>;
    listBarberProfilesUseCase = moduleFixture.get(
      ListBarberProfilesUseCase,
    ) as jest.Mocked<ListBarberProfilesUseCase>;
    getBarberProfileUseCase = moduleFixture.get(
      GetBarberProfileUseCase,
    ) as jest.Mocked<GetBarberProfileUseCase>;
  });
  ...
  describe('POST /tenants/:tenantId/barber-profiles', () => {
    it('deve retornar 201 e o perfil criado', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(createBarberProfileUseCase.run).toHaveBeenCalledWith(
            tenantId,
    ...
    it('deve retornar 400 quando regra de negócio violada', () => {
      createBarberProfileUseCase.run.mockRejectedValue(
  ...
  describe('GET /tenants/:tenantId/barber-profiles/:id', () => {
    it('deve retornar 200 e o perfil quando existe', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body).toHaveProperty('id', profileId);
          expect(res.body).toHaveProperty('displayName', 'João Barbeiro');
          expect(getBarberProfileUseCase.run).toHaveBeenCalledWith(
            tenantId,
            profileId,
          );
        });
    ...
    it('deve retornar 404 quando perfil não existe', () => {
      getBarberProfileUseCase.run.mockRejectedValue(
        new NotFoundException('Barber profile not found'),
      );
  ...
  describe('PATCH /tenants/:tenantId/barber-profiles/:id', () => {
    it('deve retornar 200 e o perfil atualizado', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(updateBarberProfileUseCase.run).toHaveBeenCalledWith(
            tenantId,
            profileId,
    ...
    it('deve retornar 404 quando perfil não existe', () => {
      updateBarberProfileUseCase.run.mockRejectedValue(
        new NotFoundException('Barber profile not found'),
      );
  ...
  describe('PATCH /tenants/:tenantId/barber-profiles/:id/deactivate', () => {
    it('deve retornar 200 e o perfil desativado', () => {
      ...
      return request(app.getHttpServer())
        .patch(
          `/tenants/${tenantId}/barber-profiles/${profileId}/deactivate`,
        )
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('isActive', false);
          expect(deactivateBarberProfileUseCase.run).toHaveBeenCalledWith(
            tenantId,
            profileId,
            'user-uuid-123',
          );
    ...
    it('deve retornar 404 quando perfil não existe', () => {
      deactivateBarberProfileUseCase.run.mockRejectedValue(
        new NotFoundException('Barber profile not found'),
      );
