...
describe('BarberProfileRepository', () => {
    ...
    beforeEach(async () => {
        ...
        typeOrmRepo = module.get(getRepositoryToken(BarberProfileEntity)) as jest.Mocked<Repository<BarberProfileEntity>>;
    });
    ...
    describe('findByTenantUserIdNonDeleted', () => {
        it('deve retornar perfil quando existe', async () => {
            ...
            const result = await repository.findByTenantUserIdNonDeleted(tenantId, tenantUserId);
            expect(typeOrmRepo.createQueryBuilder).toHaveBeenCalledWith('bp');
            expect(mockQueryBuilder.where).toHaveBeenCalledWith('bp.tenant_id = :tenantId', { tenantId });
            expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('bp.tenant_user_id = :tenantUserId', { tenantUserId });
            expect(result).toEqual(mockProfile);
        });
    });
    ...
    describe('update', () => {
        it('deve atualizar e retornar entidade', async () => {
            ...
            expect(typeOrmRepo.update).toHaveBeenCalledWith({ id: profileId, tenantId }, { displayName: 'Nome Novo' });
            expect(typeOrmRepo.findOne).toHaveBeenCalledWith({
                where: { id: profileId, tenantId },
                withDeleted: false,
            });
