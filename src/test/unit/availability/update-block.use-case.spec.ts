...
describe('UpdateBlockUseCase', () => {
    ...
    it('lança NotFound quando bloco não existe', async () => {
        ...
        await expect(useCase.run(tenantId, barberProfileId, blockId, { startTime: '12:00', endTime: '13:00' }, userId, TenantUserRole.ADMIN)).rejects.toThrow(NotFoundException);
    });
    it('lança BLOCK_REASON_RESERVED quando motivo é BOOKING', async () => {
        ...
        await expect(useCase.run(tenantId, barberProfileId, blockId, { startTime: '12:00', endTime: '13:00' }, userId, TenantUserRole.ADMIN)).rejects.toThrow(BusinessRuleException);
    });
    it('atualiza bloco comum', async () => {
        const updated = await useCase.run(tenantId, barberProfileId, blockId, { startTime: '12:30', endTime: '13:30' }, userId, TenantUserRole.ADMIN);
