...
describe('CreateProfessionalServiceLinkUseCase', () => {
    ...
    beforeEach(async () => {
        availabilityRepository = {
            findProfessionalServiceLinkByProfessionalAndService: jest.fn().mockResolvedValue(null),
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: TENANT_PROFESSIONAL_REPOSITORY, useValue: tenantProfessionalRepository },
                {
    ...
    it('cria vínculo quando serviço ativo e não duplicado', async () => {
        const result = await useCase.run(tenantId, tenantProfessionalId, { serviceId }, userId, TenantUserRole.ADMIN);
        expect(serviceRepository.findById).toHaveBeenCalledWith(serviceId, tenantId);
        expect(availabilityRepository.createProfessionalServiceLink).toHaveBeenCalledWith({
            tenantId,
            tenantProfessionalId,
            serviceId,
        });
    ...
    it('lança quando serviço não existe', async () => {
        ...
        await expect(useCase.run(tenantId, tenantProfessionalId, { serviceId }, userId, TenantUserRole.ADMIN)).rejects.toThrow(BusinessRuleException);
    });
    ...
    it('lança PROFESSIONAL_SERVICE_ALREADY_EXISTS quando já existe vínculo', async () => {
        availabilityRepository.findProfessionalServiceLinkByProfessionalAndService.mockResolvedValue(mockLink);
        await expect(useCase.run(tenantId, tenantProfessionalId, { serviceId }, userId, TenantUserRole.ADMIN)).rejects.toThrow(BusinessRuleException);
    });
});
