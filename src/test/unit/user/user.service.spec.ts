...
describe('UserService', () => {
    ...
    beforeEach(async () => {
        ...
        userSyncService = module.get(UserSyncService) as jest.Mocked<UserSyncService>;
    });
    ...
    describe('validateUserExists', () => {
        ...
        it('deve lançar UnauthorizedException quando o usuário não existe no banco', async () => {
            repo.findByFirebaseUid.mockResolvedValue(null);
            await expect(service.validateUserExists('uid-inexistente')).rejects.toThrow(UnauthorizedException);
        });
        it('deve lançar ForbiddenException quando o usuário não está ACTIVE', async () => {
            ...
            await expect(service.validateUserExists('firebase-uid-1')).rejects.toThrow(ForbiddenException);
        });
    });
    describe('syncUserWithFirebase', () => {
        it('deve delegar para userSyncService.syncFromFirebase', async () => {
            ...
            expect(userSyncService.syncFromFirebase).toHaveBeenCalledWith('firebase-uid-1');
            expect(result).toEqual(mockUser);
        });
    });
    ...
    describe('create', () => {
        ...
        it('deve criar usuário, hashear senha, sync Firebase e setar firebaseUid', async () => {
            ...
            expect(repo.setFirebaseUid).toHaveBeenCalledWith('uuid-123', 'new-firebase-uid');
        ...
        it('deve fazer soft delete do usuário criado se createInFirebase falhar', async () => {
            repo.findByEmail.mockResolvedValue(null);
            repo.create.mockResolvedValue({ ...mockUser, firebaseUid: null });
            userSyncService.createInFirebase.mockRejectedValue(new Error('Firebase error'));
            await expect(service.create({
                email: 'novo@email.com',
                name: 'Novo User',
                password: 'senha123',
    ...
    describe('update', () => {
        it('deve lançar NotFoundException quando o usuário não existe', async () => {
            repo.findById.mockResolvedValue(null);
            await expect(service.update('id-inexistente', { name: 'Nome' })).rejects.toThrow(NotFoundException);
        ...
        it('deve atualizar no banco e sincronizar no Firebase quando usuário tem firebaseUid', async () => {
            ...
            expect(userSyncService.syncToFirebase).toHaveBeenCalledWith('firebase-uid-1', { displayName: 'Nome Atualizado' });
        ...
        it('deve hashear nova senha quando password é informado', async () => {
            ...
            expect(repo.update).toHaveBeenCalledWith('uuid-123', expect.objectContaining({
                passwordHash: '$2a$10$hashed',
            }));
        });
    });
    describe('delete', () => {
        ...
        it('deve chamar disableInFirebase e softDelete quando usuário tem firebaseUid', async () => {
            ...
            expect(userSyncService.disableInFirebase).toHaveBeenCalledWith('firebase-uid-1');
            expect(repo.softDelete).toHaveBeenCalledWith('uuid-123');
        });
