...
describe('TenantController (e2e)', () => {
    ...
    beforeAll(async () => {
        mockRepository.findOne.mockImplementation((args: {
            where: Record<string, string>;
        }) => {
            const where = args?.where || {};
            if (where.slug === 'barbearia-do-vitinho' ||
    ...
    describe('GET /tenants/:slug', () => {
        ...
        it('deve retornar 200 com body vazio quando o slug não existe', async () => {
            const res = await request(app.getHttpServer()).get('/tenants/slug-inexistente');
