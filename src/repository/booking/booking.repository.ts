...
import { CreateBookingDraftData, IBookingRepository, } from '../../modules/booking/interfaces/booking-repository.interface';
...
export class BookingRepository implements IBookingRepository {
    ...
    async createDraft(data: CreateBookingDraftData): Promise<BookingEntity> {
        return this.dataSource.transaction(async (manager) => {
            await manager.query(`SELECT id FROM barber_profiles WHERE id = $1 AND tenant_id = $2 FOR UPDATE`, [data.barberProfileId, data.tenantId]);
            const qb = manager
    ...
    async findByIdForBarber(id: string, tenantId: string, barberProfileId: string): Promise<BookingEntity | null> {
        return this.bookingRepo.findOne({
            where: { id, tenantId, barberProfileId },
        });
    }
    async updateStatus(id: string, tenantId: string, barberProfileId: string, expectedStatus: BookingStatus, newStatus: BookingStatus): Promise<BookingEntity> {
        return this.dataSource.transaction(async (manager) => {
            await manager.query(`SELECT id FROM barber_profiles WHERE id = $1 AND tenant_id = $2 FOR UPDATE`, [barberProfileId, tenantId]);
