...
export class TenantUserRepository implements ITenantUserRepository {
    ...
    async findByIdAndTenant(id: string, tenantId: string): Promise<TenantUserEntity | null> {
        return this.repo.findOne({ where: { id, tenantId } });
    }
    async findByTenantAndUser(tenantId: string, userId: string): Promise<TenantUserEntity | null> {
