...
@Injectable()
export abstract class BaseRepository<Entity extends {
    id: string;
    tenantId: string;
}, Create, Update> implements ICrudRepository<Entity, Create, Update> {
    protected abstract getTableName(): string;
    protected abstract getRepository(): Repository<Entity>;
    abstract findPaginated(options: PaginatedOptionsDto, tenantId: string): Promise<PaginatedResponseDto<Entity>>;
    ...
    async update(id: string, data: Update, tenantId: string): Promise<Entity> {
        ...
        const repository = this.getRepository();
        await repository.update({ id, tenantId } as any, data as unknown as Partial<Repository<Entity>['update']>);
