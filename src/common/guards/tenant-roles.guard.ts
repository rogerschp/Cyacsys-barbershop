...
export class TenantRolesGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!tenant?.id) {
            throw new TenantForbiddenException('TENANT_NOT_RESOLVED', 'Tenant not resolved. Use TenantInterceptor and TenantMembershipGuard first.', { path });
        }
        if (!membership) {
            ...
            throw new TenantForbiddenException('NO_MEMBERSHIP', 'User is not a member of this tenant.', { tenantId: tenant.id, path });
        }
        ...
        if (!hasRole) {
            ...
            throw new TenantForbiddenException('INSUFFICIENT_ROLE', `Role ${membership.role} cannot access this resource. Required: ${requiredRoles.join(' or ')}.`, { tenantId: tenant.id, path });
        }
        return true;
    }
