...
import { AuthenticationError, RoleNotAuthorizedException, } from '../utils/auth-exceptions';
...
export class UserRolesGuard implements CanActivate {
    ...
    private resolveUserRole(context: ExecutionContext, requestUser: RequestWithUserRole['user'], requiredRoles: Role[]): Role {
        const roleFromDb = requestUser?.dbUser?.role;
        if (roleFromDb && Object.values(Role).includes(roleFromDb)) {
            return roleFromDb;
        }
        throw new RoleNotAuthorizedException(requiredRoles, requestUser?.dbUser?.role ?? 'none');
    }
    private validateUserRole(userRole: Role, requiredRoles: Role[]): void {
        const hasRequiredRole = requiredRoles.includes(userRole);
        if (!hasRequiredRole) {
            this.logger.warn(`Access denied: User role '${userRole}' not in required roles [${requiredRoles.join(', ')}]`);
