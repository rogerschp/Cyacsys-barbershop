...
import { IFirebaseUserSync, CreateFirebaseUserInput, UpdateFirebaseUserInput, FIREBASE_USER_SYNC, } from '../firebase/interfaces/firebase-user-sync.interface';
...
export class UserSyncService {
    ...
    async syncToFirebase(firebaseUid: string, payload: UpdateFirebaseUserInput): Promise<void> {
        if (Object.keys(payload).length === 0)
            return;
        await this.firebaseUserSync.updateUser(firebaseUid, payload);
    }
    async createInFirebase(input: CreateFirebaseUserInput): Promise<{
        uid: string;
    }> {
