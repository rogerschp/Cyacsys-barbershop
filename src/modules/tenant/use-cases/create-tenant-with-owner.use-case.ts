...
import { normalizeSlug, isValidSlugFormat, } from '../../../common/utils/slug.utils';
...
export class CreateTenantWithOwnerUseCase {
    constructor(
    @InjectDataSource()
    private readonly dataSource: DataSource, private readonly tenantRepository: TenantRepository) { }
    async run(userId: string, dto: CreateTenantDto): Promise<TenantEntity> {
        ...
        if (!isValidSlugFormat(slug)) {
            throw new BadRequestException('Slug must be 3-100 chars, lowercase letters, numbers and single hyphens');
        }
