    ...
    onModuleInit(): void {
        ...
        if (base64?.trim()) {
            try {
                ...
                this.logger.log('Firebase Admin initialized with FIREBASE_SERVICE_ACCOUNT_BASE64');
            }
            catch (error) {
                this.logger.error('Invalid FIREBASE_SERVICE_ACCOUNT_BASE64 (must be base64-encoded service account JSON)', error);
                throw error;
            }
        }
        else {
