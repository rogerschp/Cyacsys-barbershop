...
import { IFirebaseUserSync, CreateFirebaseUserInput, UpdateFirebaseUserInput, } from './interfaces/firebase-user-sync.interface';
@Injectable()
export class FirebaseUserSyncService implements IFirebaseUserSync {
    ...
    async createUser(input: CreateFirebaseUserInput): Promise<{
        ...
        try {
            ...
            this.logger.log(`Firebase user created: ${userRecord.uid} (${input.email})`);
            ...
            const isEmailExists = code === 'auth/email-already-exists' ||
                message?.includes('email already exists') ||
                message?.includes('already in use');
            if (isEmailExists) {
                this.logger.warn(`Firebase user already exists for ${input.email}`);
                throw new ConflictException('Email already registered in Firebase. User must be created through the system (DB first, then sync).');
            }
