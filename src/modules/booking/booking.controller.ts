import { Body, Controller, Param, Patch, Post, Req, UseGuards, UseInterceptors, } from '@nestjs/common';
...
export class BookingController {
    constructor(private readonly createBookingDraftUseCase: CreateBookingDraftUseCase, private readonly confirmBookingUseCase: ConfirmBookingUseCase, private readonly cancelBookingDraftUseCase: CancelBookingDraftUseCase) { }
    @Post('draft')
    @TenantRoles(...BOOKING_ROLES)
    @ApiOperation({
        summary: 'Cria rascunho de agendamento (segura o slot até confirmar ou cancelar)',
    })
    ...
    req: RequestWithUserAndMembership) {
        return this.createBookingDraftUseCase.run(tenantId, barberProfileId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.confirmBookingUseCase.run(tenantId, barberProfileId, bookingId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.cancelBookingDraftUseCase.run(tenantId, barberProfileId, bookingId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
}
