...
export class ConfirmBookingUseCase {
    ...
    async run(tenantId: string, barberProfileId: string, bookingId: string, userId: string, callerRole?: string): Promise<BookingEntity> {
        ...
        if (booking.status !== BookingStatus.DRAFT) {
            throw new BusinessRuleException('BOOKING_INVALID_STATUS', 'Só é possível confirmar um agendamento em rascunho.');
        }
        const nowUtc = DateTime.now().toUTC();
        const startUtc = DateTime.fromJSDate(booking.startsAt).toUTC();
        if (startUtc <= nowUtc) {
            throw new BusinessRuleException('BOOKING_IN_THE_PAST', 'Não é possível confirmar: o horário de início já passou ou é o instante atual.');
        }
        try {
            return await this.bookingRepository.updateStatus(bookingId, tenantId, barberProfileId, BookingStatus.DRAFT, BookingStatus.CONFIRMED);
        }
        catch (e: unknown) {
            if (e instanceof Error &&
                e.message === 'BOOKING_SLOT_CONFLICT') {
                throw new BusinessRuleException('SLOT_NOT_AVAILABLE', 'Outro agendamento ocupou este horário. Cancele o rascunho e escolha outro slot.');
            }
            ...
            if (e instanceof QueryFailedError) {
                const code = (e as QueryFailedError & {
                    driverError?: {
                        code?: string;
                    };
                })
                    .driverError?.code;
                if (code === '23505') {
                    throw new BusinessRuleException('SLOT_NOT_AVAILABLE', 'Outro agendamento ocupou este horário.');
                }
            }
            throw e;
        }
