...
export class AvailabilityController {
    constructor(private readonly createBarberServiceLinkUseCase: CreateBarberServiceLinkUseCase, private readonly updateBarberServiceLinkUseCase: UpdateBarberServiceLinkUseCase, private readonly deleteBarberServiceLinkUseCase: DeleteBarberServiceLinkUseCase, private readonly listBarberServiceLinksUseCase: ListBarberServiceLinksUseCase, private readonly createWorkingHoursUseCase: CreateWorkingHoursUseCase, private readonly updateWorkingHoursUseCase: UpdateWorkingHoursUseCase, private readonly deleteWorkingHoursUseCase: DeleteWorkingHoursUseCase, private readonly listWorkingHoursUseCase: ListWorkingHoursUseCase, private readonly getWorkingHoursUseCase: GetWorkingHoursUseCase, private readonly createWorkingHoursPeriodUseCase: CreateWorkingHoursPeriodUseCase, private readonly updateWorkingHoursPeriodUseCase: UpdateWorkingHoursPeriodUseCase, private readonly deleteWorkingHoursPeriodUseCase: DeleteWorkingHoursPeriodUseCase, private readonly createTimeOffUseCase: CreateTimeOffUseCase, private readonly updateTimeOffUseCase: UpdateTimeOffUseCase, private readonly deleteTimeOffUseCase: DeleteTimeOffUseCase, private readonly listTimeOffsUseCase: ListTimeOffsUseCase, private readonly createBlockUseCase: CreateBlockUseCase, private readonly updateBlockUseCase: UpdateBlockUseCase, private readonly deleteBlockUseCase: DeleteBlockUseCase, private readonly listBlocksUseCase: ListBlocksUseCase, private readonly getAvailableSlotsUseCase: GetAvailableSlotsUseCase) { }
    @Get('available-slots')
    ...
    req: RequestWithUserAndMembership) {
        ...
        return this.getAvailableSlotsUseCase.run(tenantId, barberProfileId, query.serviceId, query.date, userId, callerRole);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.listBarberServiceLinksUseCase.run(tenantId, barberProfileId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.createBarberServiceLinkUseCase.run(tenantId, barberProfileId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.updateBarberServiceLinkUseCase.run(tenantId, barberProfileId, linkId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.deleteBarberServiceLinkUseCase.run(tenantId, barberProfileId, linkId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    @Get('working-hours')
    @TenantRoles(...AGENDA_ROLES)
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.listWorkingHoursUseCase.run(tenantId, barberProfileId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    @Post('working-hours')
    @TenantRoles(...AGENDA_ROLES)
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.createWorkingHoursUseCase.run(tenantId, barberProfileId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.getWorkingHoursUseCase.run(tenantId, barberProfileId, workingHoursId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.updateWorkingHoursUseCase.run(tenantId, barberProfileId, workingHoursId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        await this.deleteWorkingHoursUseCase.run(tenantId, barberProfileId, workingHoursId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.createWorkingHoursPeriodUseCase.run(tenantId, barberProfileId, workingHoursId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.updateWorkingHoursPeriodUseCase.run(tenantId, barberProfileId, workingHoursId, periodId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        await this.deleteWorkingHoursPeriodUseCase.run(tenantId, barberProfileId, workingHoursId, periodId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    @Get('time-offs')
    @TenantRoles(...AGENDA_ROLES)
    async listTimeOffs(
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.listTimeOffsUseCase.run(tenantId, barberProfileId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    @Post('time-offs')
    @TenantRoles(...AGENDA_ROLES)
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.createTimeOffUseCase.run(tenantId, barberProfileId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.updateTimeOffUseCase.run(tenantId, barberProfileId, timeOffId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.deleteTimeOffUseCase.run(tenantId, barberProfileId, timeOffId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    @Get('blocks')
    @TenantRoles(...AGENDA_ROLES)
    async listBlocks(
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.listBlocksUseCase.run(tenantId, barberProfileId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    @Post('blocks')
    @TenantRoles(...AGENDA_ROLES)
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.createBlockUseCase.run(tenantId, barberProfileId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.updateBlockUseCase.run(tenantId, barberProfileId, blockId, dto, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
    ...
    @Req()
    req: RequestWithUserAndMembership) {
        return this.deleteBlockUseCase.run(tenantId, barberProfileId, blockId, req.user?.dbUser?.id ?? '', req.tenantMembership?.role);
    }
}
