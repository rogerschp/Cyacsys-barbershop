...
export class CreateProfessionalServiceLinkUseCase {
  ...
  async run(tenantId: string, tenantProfessionalId: string, dto: CreateProfessionalServiceLinkDto, userId: string, callerRole?: string): Promise<ProfessionalServiceLinkEntity> {
        ...
        const existing = await this.availabilityRepository.findProfessionalServiceLinkByProfessionalAndService(tenantProfessionalId, tenantId, dto.serviceId);
        if (existing) {
            throw new BusinessRuleException('PROFESSIONAL_SERVICE_ALREADY_EXISTS', 'Este serviço já está vinculado ao profissional.');
        }
