  ...
  async run(
    ...
    if (!barber.isActive) {
      throw new BusinessRuleException(
        'BARBER_INACTIVE',
        'Barbeiro inativo não possui slots disponíveis.',
      );
    }
    ...
    const link =
      await this.availabilityRepository.findBarberServiceLinkByBarberAndService(
        barberProfileId,
        tenantId,
        serviceId,
      );
    if (!link || !link.isActive) {
      throw new BusinessRuleException(
        'BARBER_SERVICE_NOT_OFFERED',
        'O barbeiro não oferece este serviço.',
      );
    }

    const wh = await this.availabilityRepository.findWorkingHoursByBarberAndDay(
      barberProfileId,
      tenantId,
      dow,
      true,
    ...
    for (const period of periods) {
      ...
      while (t + duration <= pe) {
        ...
        if (!slotStartUtc.isValid || slotStartUtc < nowUtc) {
          t += duration;
          continue;
        }
