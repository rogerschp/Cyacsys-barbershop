...
export class UpdateWorkingHoursUseCase {
  ...
  async run(
    ...
    const existing = await ensureWorkingHoursForTenantProfessional(
    ...
    if (nextActive) {
      const count =
        await this.availabilityRepository.countActivePeriodsByWorkingHoursId(
          workingHoursId,
        );
      if (count < 1) {
        throw new BusinessRuleException(
    ...
    if (dto.dayOfWeek === undefined && dto.isActive === undefined) {
      const full = await this.availabilityRepository.findWorkingHoursById(
        workingHoursId,
        tenantId,
        true,
      );
      if (!full) throw new NotFoundException('Working hours not found');
      return full;
    }
    return this.availabilityRepository.updateWorkingHours(
      workingHoursId,
      tenantId,
      {
