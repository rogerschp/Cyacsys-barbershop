...
export async function assertBarberAgendaAccess(params: {
    ...
    if (callerRole !== TenantUserRole.BARBER) {
        throw new TenantForbiddenException('FORBIDDEN', 'Sem permissão para gerenciar a agenda deste barbeiro.', { tenantId });
    }
    const profile = await barberProfileRepository.findById(barberProfileId, tenantId);
    if (!profile) {
