...
export class FirebaseAuthProvider implements IAuthProvider {
    ...
    private parseExpiresIn(value: string | number | undefined, defaultValue: number): number {
        if (value === undefined || value === null) {
            return defaultValue;
        }
        return typeof value === 'string' ? parseInt(value, 10) : Number(value);
    }
    async authenticateWithCredentials(authLoginDto: AuthLoginDto): Promise<AuthLoginResponseDto> {
        const apiKey = this.getApiKey();
        const { email, password } = authLoginDto;
        try {
            const response = await axios.post<FirebaseSignInResponse>(`${FIREBASE_SIGN_IN_URL}?key=${apiKey}`, { email, password, returnSecureToken: true }, { headers: { 'Content-Type': 'application/json' } });
    ...
    async refreshToken(refreshTokenDto: AuthRefreshDto): Promise<AuthRefreshResponseDto> {
        const apiKey = this.getApiKey();
        try {
            ...
            const response = await axios.post<FirebaseRefreshTokenResponse>(`${FIREBASE_TOKEN_URL}?key=${apiKey}`, params.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
            ...
            const expiresIn = this.parseExpiresIn(data.expires_in, DEFAULT_EXPIRES_IN_SECONDS);
