...
export class ServiceController {
    constructor(private readonly createServiceUseCase: CreateServiceUseCase, private readonly updateServiceUseCase: UpdateServiceUseCase, private readonly deactivateServiceUseCase: DeactivateServiceUseCase, private readonly listServicesByTenantUseCase: ListServicesByTenantUseCase, private readonly getServiceUseCase: GetServiceUseCase) { }
    @Post()
    @TenantRoles(TenantUserRole.OWNER, TenantUserRole.ADMIN)
    @ApiOperation({
        summary: 'Cria um serviço no tenant',
        description: 'Apenas OWNER ou ADMIN. Nome único por tenant. Preço >= 0, duração >= 5 min.',
    })
    ...
    @ApiResponse({
        status: 400,
        description: 'Regra de negócio violada (nome duplicado, preço/duração inválido)',
    })
    ...
    @ApiOperation({
        ...
        description: 'Não permite alterar tenantId. Valida nome único e preço/duração.',
    })
    @ApiParam({ name: 'tenantId', description: 'UUID do tenant' })
    @ApiParam({ name: 'id', description: 'UUID do serviço' })
    ...
    @ApiOperation({
        ...
        description: 'Define isActive = false. Serviço inativo não pode ser usado em booking.',
    })
    @ApiParam({ name: 'tenantId', description: 'UUID do tenant' })
    @ApiParam({ name: 'id', description: 'UUID do serviço' })
    @ApiResponse({
