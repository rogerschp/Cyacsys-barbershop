...
export class UpdateServiceUseCase {
    ...
    async run(tenantId: string, serviceId: string, dto: UpdateServiceDto, performedBy?: string): Promise<ServiceEntity> {
        ...
        if (dto.name !== undefined) {
            ...
            const duplicate = await this.serviceRepository.findNonDeletedByName(tenantId, normalizedName, serviceId);
            if (duplicate) {
        ...
        const updated = await this.serviceRepository.update(serviceId, tenantId, updates);
        this.logger.log({
