...
export class BarberProfileController {
  constructor(
    private readonly createBarberProfileUseCase: CreateBarberProfileUseCase,
    private readonly updateBarberProfileUseCase: UpdateBarberProfileUseCase,
    private readonly deactivateBarberProfileUseCase: DeactivateBarberProfileUseCase,
    private readonly listBarberProfilesUseCase: ListBarberProfilesUseCase,
    private readonly getBarberProfileUseCase: GetBarberProfileUseCase,
  ) {}
  @Post()
  ...
  @ApiResponse({
    status: 400,
    description:
      'Regra de negócio (INVALID_BARBER_ROLE, BARBER_PROFILE_ALREADY_EXISTS, INVALID_EXPERIENCE_YEARS)',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Define isActive = false. Barbeiro inativo não pode receber agendamentos (validação no módulo Booking).',
  })
  @ApiParam({ name: 'tenantId', description: 'UUID do tenant' })
  @ApiParam({ name: 'id', description: 'UUID do perfil de barbeiro' })
  @ApiResponse({
