...
export class CreateBarberProfileUseCase {
    ...
    async run(tenantId: string, dto: CreateBarberProfileDto, createdBy: string): Promise<BarberProfileEntity> {
        const tenantUser = await this.tenantUserService.getByIdAndTenant(dto.tenantUserId, tenantId);
        if (tenantUser.role !== TenantUserRole.BARBER) {
            throw new BusinessRuleException('INVALID_BARBER_ROLE', 'O usuário vinculado deve ter role BARBER.', { tenantUserId: dto.tenantUserId });
        }
        const existing = await this.barberProfileRepository.findByTenantUserIdNonDeleted(tenantId, dto.tenantUserId);
        if (existing) {
            throw new BusinessRuleException('BARBER_PROFILE_ALREADY_EXISTS', 'Já existe um perfil de barbeiro para este usuário neste tenant.', { tenantUserId: dto.tenantUserId });
        }
