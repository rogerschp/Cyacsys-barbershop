...
export class DeactivateBarberProfileUseCase {
    ...
    async run(tenantId: string, barberProfileId: string, performedBy: string): Promise<BarberProfileEntity> {
        ...
        const updated = await this.barberProfileRepository.update(barberProfileId, tenantId, { isActive: false });
        this.logger.log({
